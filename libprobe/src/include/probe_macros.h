...
#define FOR_EACH0(action, ...)                                                 \
	GET_MACRO(_0, __VA_ARGS__, FE_13, FE_12, FE_11, FE_10, FE_9, FE_8,     \
...
#define FOR_EACH1(action, _ctx, ...)                                           \
	GET_MACRO(_0, __VA_ARGS__, FE1_13, FE1_12, FE1_11, FE1_10, FE1_9,      \
		  FE1_8, FE1_7, FE1_6, FE1_5, FE1_4, FE1_3, FE1_2, FE1_1,      \
		  FE1_0)                                                       \
...
#define FOR_EACH2(action, _ctx, _x, ...)                                       \
	GET_MACRO(_0, __VA_ARGS__, FE2_13, FE2_12, FE2_11, FE2_10, FE2_9,      \
		  FE2_8, FE2_7, FE2_6, FE2_5, FE2_4, FE2_3, FE2_2, FE2_1,      \
		  FE2_0)                                                       \
...
#define DECLARE_HANDLER(name)                                                  \
	typedef void name##_event_handler(void *ctx,                           \
					  struct bpf_##name##_event_t e);      \
...
#define DECLARE_RULE_FLUSHER(name)                                             \
	...
	void flush_##name##_rejection_rule(                                    \
		struct state *s, struct query_bpf_##name##_event_t rule)       \
	{                                                                      \
		bpf_map_update_elem(                                           \
			bpf_map__fd(s->obj->maps.name##_rejections),           \
			&name##_rejections_size, &rule, BPF_ANY);              \
		name##_rejections_size++;                                      \
		unsigned int index = name##_index;                             \
		bpf_map_update_elem(                                           \
			bpf_map__fd(s->obj->maps.rejection_rule_sizes),        \
			&index, &name##_rejections_size, BPF_ANY);             \
	}                                                                      \
                                                                               \
	void flush_##name##_filter_rule(                                       \
		struct state *s, struct query_bpf_##name##_event_t rule)       \
	{                                                                      \
		...
		unsigned int index = name##_index;                             \
		bpf_map_update_elem(                                           \
			bpf_map__fd(s->obj->maps.filter_rule_sizes), &index,   \
