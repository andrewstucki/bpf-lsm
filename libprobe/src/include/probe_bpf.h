...
get_or_create_cached_process(struct task_struct *task)
{
	...
	bpf_map_update_elem(&processes, &pid, &empty_cached_process,
			    BPF_NOEXIST);
...
INLINE_STATIC struct cached_file *get_or_create_cached_file(struct inode *inode)
{
...
#define __check_rejection_filter(m, p, e, r)                                   \
	...
	if (r == 0) { /* don't override what the user has set */               \
		...
		if (size && *size > 0) {                                       \
			if (___check_##m(*size, &m##_rejections,               \
					 &event->m##_event_t)) {               \
...
#define __basic_process_info_for_task(x, task, ...)                            \
	...
	x.start = adjust_timestamp(                                            \
		BPF_CORE_READ(task, ##__VA_ARGS__, start_time))
...
#define __copy_cached_process(x, cached)                                       \
	...
	_Pragma("unroll") for (int i = 0;                                      \
		...
		memcpy(x.args[i], cached->args[i], ARGSIZE)
	...
	{                                                                      \
		...
		struct bpf_event_t *event =                                    \
			bpf_ringbuf_reserve(&events,                           \
					    sizeof(struct bpf_event_t),        \
					    RINGBUFFER_FLAGS);                 \
		if (event) {                                                   \
			event->type = type_##module##_event_t;                 \
			struct bpf_##module##_event_t *e =                     \
			...
			e->user.group.id =                                     \
				BPF_CORE_READ(c, real_cred, gid.val);          \
			e->user.effective.id =                                 \
				BPF_CORE_READ(c, cred, uid.val);               \
			e->user.effective.group.id =                           \
				BPF_CORE_READ(c, cred, gid.val);               \
                                                                               \
			...
			if ((cached = get_cached_process(c))) {                \
				__copy_cached_process(e->process, cached);     \
                                                                               ...
                                                                               \
			__basic_process_info_for_task(e->process.parent, c,    \
						      real_parent);            \
			if ((cached = get_cached_process(                      \
				     BPF_CORE_READ(c, real_parent)))) {        \
				__copy_cached_process(e->process.parent,       \
						      cached);                 \
			}                                                      \
                                                                               \
			_Pragma("GCC diagnostic push") _Pragma(                \
				...
				__ret = ____##module(                          \
					___bpf_ctx_cast(__VA_ARGS__), e, c);   \
			_Pragma("GCC diagnostic pop")                          \
				__check_rejection_filter(module, prefix, e,    \
							 __ret);               \
